...
import type { ApiResponse, SettingsImportPreviewResult } from '@/types/api'
...
export async function POST(_req: NextRequest): Promise<NextResponse<ApiResponse<SettingsImportPreviewResult>>> {
  const requestId = crypto.randomUUID()
  try {
    const data = await getService().previewImportSettings()
