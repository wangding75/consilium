...
export async function POST(_req: NextRequest): Promise<NextResponse<ApiResponse<SettingsImportCommitResult>>> {
  const requestId = crypto.randomUUID()
  try {
    const data = await getService().importSettings({} as SettingsImportCommitRequest)
