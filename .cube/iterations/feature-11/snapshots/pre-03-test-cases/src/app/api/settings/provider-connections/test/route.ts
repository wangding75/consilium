...
export async function POST(_req: NextRequest): Promise<NextResponse<ApiResponse<ProviderConnectionTestResult>>> {
  const requestId = crypto.randomUUID()
  try {
    const data = await getService().testProviderConnection({} as ProviderConnectionTestRequest)
