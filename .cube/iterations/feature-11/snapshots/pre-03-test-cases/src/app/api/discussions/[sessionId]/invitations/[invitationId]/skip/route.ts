...
import type { ApiResponse, SkipInvitationResult, SkipInvitationRequest } from '@/types/api'
...
): Promise<NextResponse<ApiResponse<SkipInvitationResult>>> {
  const requestId = crypto.randomUUID()
  try {
    const { sessionId, invitationId } = await params
    const body = (await req.json()) as Partial<SkipInvitationRequest>
    const service = createService()
    const data = await service.skipInvitation(sessionId, invitationId, {
