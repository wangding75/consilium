...
export async function POST(
  ...
  try {
    ...
    const service = createService()
    const intentResponse = body.forceAsPlainMessage
      ? undefined
      : await service.recognizeIntent(sessionId, {
        content: body.content,
        clientMessageId: body.clientMessageId,
      })
    const data = await service.sendUserMessage(sessionId, body.content, body.clientMessageId, intentResponse)
    return NextResponse.json({ success: true, data, requestId })
  } catch (err) {
    const errorCode = err instanceof ServiceError ? err.code : (err as { code?: string })?.code
    if (errorCode === 'SESSION_NOT_FOUND') {
