...
export async function PATCH(
  ...
  try {
    const body = await req.json() as UpdateProviderConnectionRequest
    const { connectionId } = await context.params
    const data = await getService().updateProviderConnection(connectionId, body)
...
export async function DELETE(
  ...
  try {
    const { connectionId } = await context.params
    const data = await getService().deleteProviderConnection(connectionId)
    return NextResponse.json({ success: true, data, requestId })
  } catch (err) {
    if (err instanceof ServiceError) {
