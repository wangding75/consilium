...
export async function POST(req: Request): Promise<NextResponse<ApiResponse<ProviderConnectionDTO>>> {
  const requestId = crypto.randomUUID()
  try {
    const body = await req.json() as CreateProviderConnectionRequest
    const data = await getService().createProviderConnection(body)
    return NextResponse.json({ success: true, data, requestId })
  } catch (err) {
