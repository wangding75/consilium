...
import type { ApiResponse, ProviderConnectionTestRequest, ProviderConnectionTestResult } from '@/types/api'
...
export async function POST(req: Request): Promise<NextResponse<ApiResponse<ProviderConnectionTestResult>>> {
  const requestId = crypto.randomUUID()
  try {
    const body = await req.json() as ProviderConnectionTestRequest
    const data = await getService().testProviderConnection(body)
